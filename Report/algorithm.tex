...
\]
A value \(F=0\) indicates a valid solution. This formulation is similar to that of previous work \cite{Wang2024}, which also iterates over columns and sub-blocks but assigns a value of 0 when a unit is valid and 1 when it contains any violation; by contrast, our fitness counts the number of duplicate digits per column and block, giving a more fine-grained penalty (e.g. a column with three duplicated entries contributes 3 rather than 1).
...
\paragraph{Mutation} For the mutation, we iterate all rows and select two non-given tiles in that row to swap their values with probability \(mutation\_rate\). This row-wise swap was inspired by swap-mutations within subgrids \cite{Sato2011, Mantere2007} and is also consistent with other work that performs swap mutations inside random rows to ensure row constraints are preserved \cite{Wang2024}. The cited paper \cite{Wang2024} additionally applies a reinitialisation mutation step, but in our implementation we only adopt the row-swap component of the mutation logic.
