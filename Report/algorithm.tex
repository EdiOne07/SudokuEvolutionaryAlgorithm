...
\While{generation < max\_generations}
  ...
  \If{first(population).fitness = $max\_fitness$} \State \Return first(population) \EndIf
  \State top $\gets$ first 20\% of Population
  \For{$i\gets 1, population\_size * 0.8$}
    \State parent1, parent2 $\gets$ sample(top)
    \State child $\gets$ crossover(parent1, parent2) \Comment{per-cell pick random parent}
    \State child $\gets$ mutate(child, mutation\_rate, initial\_sudoku) \Comment{change $mutation\_rate$ amount of random cells}
    \State next\_generation $\gets$ next\_generation + child
  \EndFor
  \For{$i\gets 1, population\_size * 0.2$}
